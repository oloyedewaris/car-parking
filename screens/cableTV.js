...
import { getTVServiceCategories, purchaseService } from "../api/utilty";
...
const CableTV = ({ navigation }) => {
  ...
  const cableTVServicesQuery = useQuery(
    ["getTVServiceCategories"],
    getTVServiceCategories
  );
  const cableTVServices = cableTVServicesQuery?.data?.data?.results;

  const handlePress = (id) => {
    setServiceId(id);
    const cableTVObject = cableTVServices?.find(
      (item) => item?.service_id === id
    );
    setServicePlans(
      cableTVObject?.variations?.map((cableTVVariation) => ({
        ...cableTVVariation,
        label: cableTVVariation?.name,
        value: cableTVVariation?.variation_code,
      }))
    );
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const dataToSubmit = {
        ...
        variation_code: serviceToUse?.variation_code,
      };
  ...
  return (
    <>
      ...
      >
        {cableTVServicesQuery?.isLoading ? (
          <View
            style={{
              width: "100%",
              height: "90%",
          ...
          <View style={{ alignItems: "stretch" }}>
            ...
            <View style={{ marginTop: 20 }}>
              ...
              <Dropdown
                ...
                value={serviceToUse}
                onChange={(service) => setServiceToUse(service)}
              />
            </View>
            ...
            <View style={{ marginTop: 20 }}>
              ...
              <TextInput
                onChangeText={(text) => formik.setFieldValue("target", text)}
      ...
      <BottomSheet ref={bottomRef} height={450}>
        ...
        >
          ...
          <View style={{ alignItems: "center" }}>
            ...
            >
              Your cable tv purchase of {serviceToUse?.label} type to{" "}
              {formik.values.target} was successful
            </Text>
          </View>
          <View>
            <TouchableOpacity

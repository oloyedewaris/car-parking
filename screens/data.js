...
import { getDataServiceCategories, purchaseService } from "../api/utilty";
...
const Data = ({ navigation }) => {
  ...
  const dataServicesQuery = useQuery(
    ["getDataServiceCategories"],
    getDataServiceCategories
  );
  const dataServices = dataServicesQuery?.data?.data?.results;

  const handlePress = (id) => {
    setServiceId(id);
    const dataObject = dataServices?.find((item) => item?.service_id === id);
    setDataPlans(
      dataObject?.variations?.map((dataVariation) => ({
        ...dataVariation,
        label: dataVariation?.name,
        value: dataVariation?.variation_code,
      }))
    );
  };
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
      const dataToSubmit = {
        ...
        phone: values?.phone,
        target: values?.phone,
        variation_code: dataToUse?.variation_code,
      };
          ...
          <View style={{ alignItems: "stretch" }}>
            ...
            <View style={{ marginTop: 20 }}>
              ...
              <Dropdown
                ...
                value={dataToUse}
                onChange={(data) => setDataToUse(data)}
              />
            </View>
      ...
      <BottomSheet ref={bottomRef} height={450}>
        ...
        >
          ...
          <View style={{ alignItems: "center" }}>
            ...
            >
              Your data of {dataToUse?.label} plan to {formik.values.phone} was
              successful
            </Text>
          </View>
          <View>
